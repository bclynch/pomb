...
import { JunctureModalComponent } from '../../modules/junctureModal/junctureModal/junctureModal';
    ...
    if (data) {
      switch (data.toLowerCase()) {
        ...
        case 'stories hub':
          ...
          break;
        case 'create trip':
          const tripModal = await this.modalCtrl.create({
            component: TripModalComponent,
            cssClass: 'tripModal',
            backdropDismiss: false
          });

          await tripModal.present();

          const { data: tripModalData } = await tripModal.onWillDismiss();
          this.tripService.handleOpenTripModal(tripModalData, null);
          break;
        case 'create juncture':
          const junctureModal = await this.modalCtrl.create({
            component: JunctureModalComponent,
            componentProps: { markerImg: this.junctureService.defaultMarkerImg },
            cssClass: 'junctureModal',
            backdropDismiss: false
          });

          await junctureModal.present();

          const { data: junctureData } = await junctureModal.onWillDismiss();
          this.junctureService.handleOpenJunctureModal(junctureData, null);
          break;
        case 'blog dashboard':
